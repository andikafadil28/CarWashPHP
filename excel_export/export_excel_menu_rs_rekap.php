...
if (!empty($where_parts)) {
    ...
    // Update info filter
    $filter_info = "MENU: " . implode(", ", $target_menus) . trim($tanggal_info);
}


// --- 2. PENGAMBILAN DATA DENGAN KLAUSA WHERE DINAMIS ---
// Query data dari database. Menghapus kolom tb_menu.nama_toko dari SELECT dan GROUP BY
$sql = "SELECT 
            tb_menu.nama, 
...
$sheet->setTitle('Laporan Penjualan Minuman');

// Tentukan Nama Kolom Terakhir (E, karena kolom Nama Toko dihapus)
$last_col = 'E';

// 1. JUDUL LAPORAN (Baris 1 & 2)
$sheet->mergeCells('A1:' . $last_col . '1');
$sheet->setCellValue('A1', 'LAPORAN REKAPITULASI PENJUALAN MINUMAN PILIHAN');
...
// Tulis Header di baris 3 (Hanya 5 kolom)
...
foreach ($data as $row) {
    ...
    $sheet->getStyle('C' . $rowNum)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_CENTER); // Total Terjual
    $sheet->getStyle('D' . $rowNum)->getNumberFormat()->setFormatCode('"Rp "#,##0'); // Harga Satuan
    $sheet->getStyle('E' . $rowNum)->getNumberFormat()->setFormatCode('"Rp "#,##0'); // Total Harga Jual
...
// Gabungkan kolom A sampai D untuk teks 'Total' (Total 4 kolom digabung)
